...
export class Signal implements EditorTypes.Signal{  
...
Signal.prototype.dispatch = function(a) {
    debugger ; 
    if (this.active) {
        const b = Array.prototype.slice.call(arguments) ; 

        let c : number = this._bindings.length ; 
        if (this.memorize) this._prevParams = b;
        if (c) {
            let d = this._bindings.slice();
            this._shouldPropagate = true ;
            do
                c--;
...
/*
export const signals = (function(h) {
